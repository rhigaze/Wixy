...
app.put('/data/:objType/:id', function (req, res) {
	...
	newObj._id = new mongodb.ObjectID(newObj._id);
...
// POST - adds 
app.post('/doUpload', upload.single('file'), function (req, res) {
    console.log('req.file', req.file);
   	let imgUrl = null;
	
    // If there is a file upload, add the url to the obj
    if (req.file) {
        imgUrl = uploadFolder + '/' + req.file.filename;
		cl(`Upload done, retuning url: `,imgUrl )
		res.json({imgUrl});
		res.end();
    } else {
		cl(`No file to upload`)
		res.json(500, { error: 'No file to upload' })
	}
});